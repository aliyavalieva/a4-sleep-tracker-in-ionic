...
We decided it would be best to see the log of user’s sleepiness directly on the same page where they log their sleepiness scale. We decided to have the date for each sleepiness on the header of each of the log history with the description in the body part - that way we could make our user side as much efficient as possible. 
